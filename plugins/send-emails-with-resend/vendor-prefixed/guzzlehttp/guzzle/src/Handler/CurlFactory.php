...
namespace ResendWP\GuzzleHttp\Handler;

use ResendWP\GuzzleHttp\Exception\ConnectException;
use ResendWP\GuzzleHttp\Exception\RequestException;
use ResendWP\GuzzleHttp\Promise as P;
use ResendWP\GuzzleHttp\Promise\FulfilledPromise;
use ResendWP\GuzzleHttp\Promise\PromiseInterface;
use ResendWP\GuzzleHttp\Psr7\LazyOpenStream;
use ResendWP\GuzzleHttp\TransferStats;
use ResendWP\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \ResendWP\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \ResendWP\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \ResendWP\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \ResendWP\GuzzleHttp\Psr7\Utils::streamFor($sink);
