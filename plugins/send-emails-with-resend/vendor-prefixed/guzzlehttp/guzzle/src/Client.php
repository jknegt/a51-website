...
namespace ResendWP\GuzzleHttp;

use ResendWP\GuzzleHttp\Cookie\CookieJar;
use ResendWP\GuzzleHttp\Exception\GuzzleException;
use ResendWP\GuzzleHttp\Exception\InvalidArgumentException;
use ResendWP\GuzzleHttp\Promise as P;
use ResendWP\GuzzleHttp\Promise\PromiseInterface;
use ResendWP\Psr\Http\Message\RequestInterface;
use ResendWP\Psr\Http\Message\ResponseInterface;
use ResendWP\Psr\Http\Message\UriInterface;
/**
 * @final
 */
class Client implements ClientInterface, \ResendWP\Psr\Http\Client\ClientInterface
{
    use ClientTrait;
    /**
    ...
    public function __construct(array $config = [])
    {
        ...
        if (isset($config['base_uri'])) {
            $config['base_uri'] = \ResendWP\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']);
        }
    ...
    public function requestAsync(string $method, $uri = '', array $options = []): PromiseInterface
    {
        ...
        $uri = $this->buildUri(\ResendWP\GuzzleHttp\Psr7\Utils::uriFor($uri), $options);
        if (\is_array($body)) {
    ...
    {
        if (isset($config['base_uri'])) {
            $uri = \ResendWP\GuzzleHttp\Psr7\UriResolver::resolve(\ResendWP\GuzzleHttp\Psr7\Utils::uriFor($config['base_uri']), $uri);
        }
    ...
    private function applyOptions(RequestInterface $request, array &$options): RequestInterface
    {
        ...
        if (isset($options['form_params'])) {
            ...
            $options['_conditional'] = \ResendWP\GuzzleHttp\Psr7\Utils::caselessRemove(['Content-Type'], $options['_conditional']);
        ...
        if (!empty($options['decode_content']) && $options['decode_content'] !== true) {
            // Ensure that we don't have the header in different case and set the new value.
            $options['_conditional'] = \ResendWP\GuzzleHttp\Psr7\Utils::caselessRemove(['Accept-Encoding'], $options['_conditional']);
        ...
        if (isset($options['body'])) {
            ...
            $modify['body'] = \ResendWP\GuzzleHttp\Psr7\Utils::streamFor($options['body']);
            unset($options['body']);
        }
        if (!empty($options['auth']) && \is_array($options['auth'])) {
            ...
            switch ($type) {
                case 'basic':
                    // Ensure that we don't have the header in different case and set the new value.
                    $modify['set_headers'] = \ResendWP\GuzzleHttp\Psr7\Utils::caselessRemove(['Authorization'], $modify['set_headers']);
        ...
        $request = \ResendWP\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify);
        if ($request->getBody() instanceof \ResendWP\GuzzleHttp\Psr7\MultipartStream) {
